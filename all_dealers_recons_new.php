...
<body>
                                ...
                                try {
                                    ...
                                    if (reconData.length > 0) {
                                        reconData.forEach((data, index) => {
                                            const pmgSalesTotal = parseInt(data.pmg_sales) +
                                                parseInt(data.hsd_sales);
                                            const avgDailySales = parseInt(pmgSalesTotal) / 38;
                                            const avgMonthlySales = parseInt(avgDailySales) * 30;
                                            const totalReceipts = parseInt(data.pmg_receipts) +
                                                parseInt(data.hsd_receipts);
                                            const totalGainLoss = parseInt(pmgSalesTotal) -
                                                parseInt(totalReceipts);
                                            ...
                                            recon_table.row.add([
                                                ...
                                                data.plan_time,
                                                splitDateTime(data
                                                    .pmg_inspection_date_current, 'date'),
                                                splitDateTime(data
                                                    .pmg_inspection_date_current, 'time'),
                                                splitDateTime(data.pmg_inspection_date_last,
                                                    'date'),
                                                splitDateTime(data.pmg_inspection_date_last,
                                                    'time'),
                                                data.no_of_days,
                                                ...
                                                data.pmg_loss_gain,
                                                splitDateTime(data
                                                    .hsd_inspection_date_current, 'date'),
                                                splitDateTime(data
                                                    .hsd_inspection_date_current, 'time'),
                                                splitDateTime(data.hsd_inspection_date_last,
                                                    'date'),
                                                splitDateTime(data.hsd_inspection_date_last,
                                                    'time'),
                                                data.no_of_days,
    ...
    function formatNumber(number) {
        if (number !== null && number !== undefined) {
            ...
            if (!isNaN(cleanNumber) && cleanNumber.trim() !== '') {
                return Number(cleanNumber).toLocaleString(); // Format the number with commas
            }
        }
        return number; // Return as is if it's not a valid number
    }

    function removeCommasFromColumns() {
        // Target the specific columns by index (index starts from 0)
        $('#recon_table tbody tr').each(function() {
            // Column 1 (index 1)
            var col1 = $(this).find('td:nth-child(2)').text();
            // Column 2 (index 2)
            // var col2 = $(this).find('td:nth-child(3)').text();

            // Remove commas and update the cell
            $(this).find('td:nth-child(2)').text(col1.replace(/,/g, ''));
            // $(this).find('td:nth-child(3)').text(col2.replace(/,/g, ''));
        });
    }
    </script>
</body>
