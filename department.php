...
<body>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel">
        <div class="offcanvas-header border-bottom">
            <h5 id="offcanvasRightLabel">Departments</h5>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#insert_form').on("submit", function (event) {
                ...
                if (update_id == 0) {
                    ...
                    $.ajax({
                        url: "<?php echo $api_url; ?>create/create_departments.php",
                        cache: false,
                        contentType: false,
                        processData: false,
                        method: "POST",
                    ...
                    $.ajax({
                        url: "<?php echo $api_url; ?>update/departments.php",
                        cache: false,
                        contentType: false,
                        processData: false,
                        method: "POST",
        ...
        function deleteData(id) {
            ...
            var settings = {
                "url": "<?php echo $api_url; ?>delete/delete_departments.php?key=03201232927&id=" + id + "",
                "method": "GET",
                "timeout": 0,
            };
        ...
        function editData(id) {
            ...
            var settings = {
                "url": "<?php echo $api_url; ?>get/get_departments.php?key=03201232927&id=" + id + "",
                "method": "GET",
                "timeout": 0,
            };
        ...
        function fetchtable() {
            ...
            // Ensure the URL is correct and that it returns the expected data
            fetch("<?php echo $api_url; ?>get/get_departments_name.php?key=03201232927&id=<?php echo $_SESSION['user_id']; ?>", requestOptions)
                .then(response => response.json())  // Parse the JSON response
                .then(response => {
                    console.log(response);  // Debugging line to see the raw response

                    // Clear the existing table data before adding new data
                    table.clear().draw();

                    // Check if response is an array and contains data
                    if (Array.isArray(response) && response.length > 0) {
                        // Loop through the response array to populate the table
                        ...
                        // If no data is returned, log the error and show an alert
                        console.log('No departments found');
                        alert('No data found');
                    }
                })
                .catch(error => {
                    console.log('error', error);  // Log the error if the fetch fails
