            ...
            if (from != '' && to != "") {
                try {
                    ...
                    const response = await fetch(
                        "<?php echo $api_url; ?>get/all_dealers_department_users.php?key=03201232927&is_role=1&user_id=" +
                        dealers,
                        requestOptions
                    );
...
// Fetch data for dropdowns and other dynamic content
function fetch_data($api_url, $endpoint, $params = '') {
    $url = "{$api_url}{$endpoint}?key=03201232927{$params}";
    $response = file_get_contents($url);
    return json_decode($response, true);
}

// Initialize variables
$dealers = fetch_data($api_url, 'get/get_asm.php', '&pre=' . $_SESSION['privilege'] . '&user_id=' . $_SESSION['user_id']);
$tm = fetch_data($api_url, 'get/get_tm.php');
$zm = fetch_data($api_url, 'get/get_zm.php');
?>
...
<head>
    <meta charset="utf-8" />
    <title>Users | <?php echo $_SESSION['user_name']; ?></title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta content="Premium Multipurpose Admin & Dashboard Template" name="description" />
    <meta content="Themesdesign" name="author" />
    ...
    <script>
        $(document).ready(function () {
            ...
            function fetchDealers() {
                $.ajax({
                    url: '<?php echo $api_url; ?>get/get_asm.php?key=03201232927&pre=<?php echo $_SESSION['privilege'] ?>&user_id=<?php echo $_SESSION['user_id'] ?>',
                    method: 'GET',
                    dataType: 'json',
                    success: function (data) {
                        $('#dealers').empty().append('<option value="">Select Dealer</option>');
                        $.each(data, function (i, item) {
                            $('#dealers').append('<option value="' + item.id + '">' + item.name + '</option>');
                        });
                    },
                    error: function (error) {
                        console.error('Error fetching data:', error);
            ...
            function getRecon() {
                ...
                if (from != '' && to != "") {
                    $.ajax({
                        url: "<?php echo $api_url; ?>get/all_dealers_department_users.php?key=03201232927&is_role=1&user_id=" + dealers,
                        method: 'GET',
                        dataType: 'json',
